...
void Si4713FrequencyNumber::on_tune_status(const tune_status_t &status) {
  float freq_mhz = static_cast<float>(status.freq) / 100.0;
  if (state != freq_mhz) {
...
void Si4713PowerNumber::on_tune_status(const tune_status_t &status) {
  float pow = static_cast<float>(status.power);
  // only publish if changed and not zero (zero means off)
  if (state != pow && pow != 0) {
...
void Si4713LevelThresholdNumber::on_property(uint16_t property, uint16_t value) {
  if ((this->is_low_thresh_ && property == SI4713_PROP_TX_ASQ_LEVEL_LOW) ||
      (!this->is_low_thresh_ && property == SI4713_PROP_TX_ASQ_LEVEL_HIGH)) {
